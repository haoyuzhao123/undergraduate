...
//add do,od,|||
%token LESS_EQUAL   GREATER_EQUAL  EQUAL   NOT_EQUAL VERTS
//end do,od,|||
//add case, default
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.' '@' '$' '#' ':'
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}'
...
Expr            :	LValue
                ...
                |   '#' Expr
                    {
                        $$.expr = new Tree.Unary(Tree.TOCOMPLEX, $2.expr, $1.loc);
                    }
                    //end the expr for COMPLEX
                ...
                |   DCOPY '(' Expr ')'
                    {
                        $$.expr = new DcopyExpr($3.expr, $1.loc);
                    }
                |   SCOPY '(' Expr ')'
                    {
                        $$.expr = new ScopyExpr($3.expr, $1.loc);
                    }
                // end DCOPY,SCOPY
                ;
...
CaseExpr        :   CASE '(' Expr ')' '{' ACaseExprList DefaultExpr '}'
                    {
                        $$.expr = new Tree.CaseExpr($3.expr, $6.elist, $7.expr, $1.loc);
                    }
                |    CASE '(' Expr ')' '{' DefaultExpr '}'
                    {
                        $$.expr = new Tree.CaseExpr($3.expr, $6.expr, $1.loc);
                    }
                ;
...
ACaseExpr       :   Constant ':' Expr ';'
                    {
                        $$.expr = new Tree.ACaseExpr($1.expr, $3.expr, $2.loc);
                    }
                ;
...
DoSubStmt       :   Expr ':' Stmt
                    {
                        $$.stmt = new DoSubStmt($1.expr, $3.stmt, $2.loc);
                    }
                    ;
