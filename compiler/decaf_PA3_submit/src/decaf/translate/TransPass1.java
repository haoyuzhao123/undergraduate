...
public class TransPass1 extends Tree.Visitor {
	...
	public void visitClassDef(Tree.ClassDef classDef) {
		...
		for (Variable v : vars) {
			...
			if (v.getType().equal(BaseType.COMPLEX)) {
				v.setImoffset(oc.next(OffsetCounter.WORD_SIZE));
			}
			//end COMPLEX
		}
	}
		...
		for (Tree.VarDef vd : funcDef.formals) {
			if (vd.symbol.getType().equal(BaseType.COMPLEX)) {
				...
				*/
				...
				t1.sym = vd.symbol;
				t1.isParam = true;
				vd.symbol.setRetemp(t1);
				vd.symbol.setOffset(oc.next(vd.symbol.getRetemp().size));
				Temp t2 = Temp.createTempI4();
				t2.sym = vd.symbol;
				t2.isParam = true;
				vd.symbol.setImtemp(t2);
				vd.symbol.setImoffset(oc.next(vd.symbol.getImtemp().size));
				//end COMPLEX
			} else {
				vd.symbol.setOrder(order++);
				Temp t = Temp.createTempI4();
	...
	public void visitVarDef(Tree.VarDef varDef) {
		...
		//System.out.println(varDef.symbol.toString());
