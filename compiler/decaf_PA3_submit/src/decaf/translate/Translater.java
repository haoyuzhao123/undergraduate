...
public class Translater {
	...
	private Tac memoOf(Function func) {
		...
		while (iter.hasNext()) {
			...
			if (v.getType().equal(BaseType.COMPLEX)) {
				Temp t1 = v.getRetemp();
				t1.offset = v.getOffset();
				sb.append(t1.name + ":" + t1.offset + " ");
				Temp t2 = v.getImtemp();
				t2.offset = v.getImoffset();
	...
	public void genCheckNum(Temp num) {
		Label exit = Label.createLabel();
		genBnez(num, exit);
	...
	//add SCOPY,DCOPY
	public Temp genDCOPY(Temp ptr, Class c) {
		Temp ret = genDirectCall(c.getNewFuncLabel(), BaseType.INT);

		Temp ptr1 = genLoadImm4(0);
		Temp ptr2 = genLoadImm4(0);
		genAssign(ptr1, ptr);
		genAssign(ptr2, ret);

		Temp step = genLoadImm4(OffsetCounter.WORD_SIZE);

		Temp store = genLoadImm4(0);

		genAssign(ptr1,genAdd(ptr1, step));
		genAssign(ptr2,genAdd(ptr2, step));
		...
		while(iter.hasNext()) {
			...
			if (sym.isVariable()) {
				if (sym.getType().isClassType()) {
					...
					Class newc = ((ClassType)sym.getType()).getSymbol().getAssociatedScope().getOwner();
					store = genLoad(ptr1, 0);
					Temp cla = genDCOPY(store, newc);
					genStore(cla, ptr2, 0);

					genAssign(ptr1,genAdd(ptr1, step));
					genAssign(ptr2,genAdd(ptr2, step));
				}
				else if (sym.getType().equal(BaseType.COMPLEX)) {
					//System.out.println("isComp");
					store = genLoad(ptr1, 0);
					genStore(store, ptr2, 0);
