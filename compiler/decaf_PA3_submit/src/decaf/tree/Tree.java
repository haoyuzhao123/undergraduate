...
public abstract class Tree {
    ...
    //add SUPER here
    public static final int SUPEREXPR = ERRONEOUS+ 1;
    //end SUPER

    //add CASE,DEFAULT here
    public static final int CASEEXPR = SUPEREXPR + 1;
    public static final int DEFAULTEXPR = CASEEXPR + 1;
    public static final int ACASEEXPR = DEFAULTEXPR + 1;
    //end CASE,DEFAULT

    //add DO,OD here
    public static final int DOSTMT = ACASEEXPR + 1;
    //end DO,OD

    //add DCOPY,SCOPY
    public static final int DCOPY = DOSTMT + 1;
    ...
    public static final int TOCOMPLEX = IM + 1;
    //end COMPLEX

    /**
     * unary operator for null reference checks, only used internally.
     */
    public static final int NULLCHK = TOCOMPLEX + 1;
    ...
    public static class Literal extends Expr {
        ...
        public Literal(int typeTag, Object value, Location loc) {
            ...
            //add COMPLEX
            if (this.typeTag == COMPLEX) {
                this.imvalue = (int)value;
                this.revalue = 0;
            }
            //end COMPLEX
            this.value = value;
    ...
    public static class DefaultExpr extends Expr {
        ...
        public DefaultExpr(Expr exp, Location loc) {
    ...
    public static class ACaseExpr extends Expr {
        ...
        public ACaseExpr(Expr cons, Expr exp, Location loc) {
            super(ACASEEXPR, loc);
            this.constant = cons;
            this.expr = exp;
        }
    ...
    public static class CaseExpr extends Expr {
        public Expr cond;
        public List<Expr> acaseexprlist;
        public Expr defaultexpr;

        public CaseExpr(Expr cond, List<Expr> lst, Expr defaultexpr, Location loc) {
            super(CASEEXPR, loc);
            this.cond = cond;
            this.acaseexprlist = lst;
            this.defaultexpr = defaultexpr;
        }

        public CaseExpr(Expr cond, Expr defaultexpr, Location loc) {
            super(CASEEXPR, loc);
            this.cond = cond;
            this.acaseexprlist = new ArrayList<Expr>();
            this.defaultexpr = defaultexpr;
        }
        ...
        public void printTo(IndentPrintWriter pw) {
            pw.println("cond");
            pw.incIndent();
            this.cond.printTo(pw);
            pw.println("cases");
            pw.incIndent();
            if (this.acaseexprlist != null) {
                for (Expr e : acaseexprlist) {
                    e.printTo(pw);
                }
            }
    ...
    public static class DoSubStmt extends Tree {
        ...
        public DoSubStmt(Expr condition, Tree stmt, Location loc) {
            super(DOSTMT, loc);
            this.condition = condition;
            this.stmt = stmt;
        }
    ...
    public static class DoStmt extends Tree {
        ...
        public DoStmt(List<Tree> stmts, Location loc) {
            super(DOSTMT, loc);
            this.stmtlist = stmts;
        }
