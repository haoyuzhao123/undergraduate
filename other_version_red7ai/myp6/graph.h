...
class Graph{
	...
	//	printf("%d\n ",k);
		if (kind){
			...
			now.ga=ga2t;// now.d2=mdt1, now.ga2
			if (tot == 0 && k == 1) now.d2=-1e9;
			else now.d2 = Solve1(tot,da,-now.p1.Score(now.r)-1);
			...
			tmp=now;
			if (tmp.n2 == 0){ now.d3=now.p1.Score(now.r); return;}
			...
			now.d3 = Solve2(tot, da, now.p1.Score(now.r));
		}	
	}
...
public:
	...
	double Prb_scr(Cardsl p1, Cardsl s1, int n2, int m2, int r){
		States tmp;
		tmp.p1=p1, tmp.s1=s1, tmp.n2=n2, tmp.m2=m2, tmp.r=r, tmp.Get_hash();
		return node[mp[tmp.hash]].d2;
	}
