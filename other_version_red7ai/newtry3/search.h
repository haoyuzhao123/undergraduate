...
const int MAX_NODES_NUMBER = 1500;
...
Move bestMove(Cards * palette, Cards hand, int player_index,
    ...
    for (int i = 0; i < max_random_time; i++) {
        ...
        if (ti > 1.9 * CLOCKS_PER_SEC) {
            //cerr << "alpha-beta random time: " << i << endl;
            break;
        }
    ...
    if (player_index == 0) {
        if (moves_arr[maxidx] == 0) cerr << "resign by best move" << endl;
        return moves_arr[maxidx];
    }
    else {
        if (moves_arr[minidx] == 0) cerr << "resign by best move" << endl;
...
double randomPlay(Cards * palette, Cards * hands, int player_index, int current_rule, int cur_player_index) {
    ...
    Cards hand = hands[cur_player_index];
    int handsize = sizeOfCards(hand);
    short count = 0;
    ...
    for (int i = 0; i < handsize; i++) {
        ...
        newpalette[0] = palette[0];
        newpalette[1] = palette[1];
        newpalette[cur_player_index] = pushBack(palette[cur_player_index], c);
        if (winning(newpalette, current_rule) == cur_player_index) {
            Cards new_palette_t[2], new_hands_t[2];
            int new_rule = moveTo(palette, hands, new_palette_t, new_hands_t,
                            cur_player_index, current_rule, c1);
            return randomPlay(new_palette_t, new_hands_t, player_index, new_rule, 1 - cur_player_index);
        }
    }

    for (int i = 0; i < handsize; i++) {
        Card c = getCard(hand, i);
        if (winning(palette, getCol(c)) == cur_player_index) {
            short colorcard = c;
            Cards new_palette_t[2], new_hands_t[2];
            int new_rule = moveTo(palette, hands, new_palette_t, new_hands_t,
                            cur_player_index, current_rule, (colorcard << 8));
            return randomPlay(new_palette_t, new_hands_t, player_index, new_rule, 1 - cur_player_index);
        }
    }
    ...
    for (int i = 0; i < handsize; i++)
        for (int j = 0; j < handsize; j++)
            if (i != j) {
                ...
                Card p = getCard(hand, i);
                Card c = getCard(hand, j);
                newpalette[cur_player_index] = pushBack(palette[cur_player_index], p);
                if (winning(newpalette, getCol(c)) == cur_player_index) {
                    short f = c;
                    short s = p;
                    Cards new_palette_t[2], new_hands_t[2];
                    int new_rule = moveTo(palette, hands, new_palette_t, new_hands_t,
                                    cur_player_index, current_rule, (f << 8 | p));
                    return randomPlay(new_palette_t, new_hands_t, player_index, new_rule, 1 - cur_player_index);
                }
            }
    return losePoint(palette, cur_player_index, current_rule);
...
Move uct(Cards * palette, Cards hand, int oppo_hand_size, int player_index, 
    ...
    for (int i = 0; i < max_random_time; i++) {
        ...
        unsigned int ti = clock();
        if (ti > 1.92 * CLOCKS_PER_SEC) {
            //cerr << "random time: " << i << endl;
            break;
        }
    ...
    for (int j = 0; j < total_moves; j++) {
        if (possible_moves[j] == 0)
            if ((abs(losePoint(palette, player_index, current_rule)) + sc[1 - player_index]) >= 40)
                continue;
    ...
    if (possible_moves[max_idx] == 0){
        cerr << "resign by uct" << endl;
    }
