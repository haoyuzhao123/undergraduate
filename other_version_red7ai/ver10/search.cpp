...
Move uct(Cards * palette, Cards hand, int oppo_hand_size, int player_index, 
            int current_rule, int max_random_time, int max_iter_time, double alpha, double (* f)(Cards *, Cards *, int, int)) {
    ...
    for (int i = 0; i < max_random_time; i++) {
        ...
        for (int j = 0; j < max_iter_time; j++) {
            ...
            score = f(tn[t].palette, tn[t].hands, tn[t].player_index, tn[t].current_rule);
...
MoveScore mcts(Cards * palette, Cards hand, int oppo_hand_size,
                 int player_index, int current_rule, int max_random_time, int max_iter_time) {
    Move moves_arr[61];
    int moves_count[61];
    double moves_val[61];
    memset(moves_arr, 0, sizeof(moves_arr));
    ...
    for (int j = 1; j < max_random_time; j++) {
        ...
        for (int k = 0; k < max_iter_time; k++) {
            ...
            double res = randomPlay(new_palette, new_hands, 1 - player_index, new_rule);
            moves_count[r] += 1;
            moves_val[r] += res;
        }
    }
