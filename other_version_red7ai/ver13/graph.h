...
class Graph{
	...
	struct States{
		...
		void Init(Cardsl _p1, Cardsl _s1, int _n2, int _m2, int _r, LL hash=-1){
		...
		if (kind){
			...
			for (i=-1;i<n1;i++){
				...
				for (j=-1;j<_n1;j++){if (i==-1 && j==-1) continue;
					...
					Dfs(tmp2,0);
					da[tot] = Data(node[tmp2].t3, node[tmp2].t1);
					da2[tot++] = Data(node[tmp2].d3, node[tmp2].t1);
					
					if (node[tmp2].t1>Eps && node[tmp2].d3>mdt1) 
						mdt1=node[tmp2].d3, ga2t=c1*100+c2;
				}
				if (i != -1) n1++;
			}
			
			now.t2 = Solve1(tot,da), now.ga=ga2t;// now.d2=mdt1, now.ga2
			now.d2= Solve1(tot,da2,-now.p1.Score(now.r));
			...
			tmp=now;
			if (tmp.n2 == 0){now.t3=1; now.d3=now.p1.Score(now.r); return;}
			for (i=1;i<=N;i++){tmp6=-1;
				...
				if (i != now.r){
					...
					if (tmp5>Eps){
						tmp2 = Get_num(tmp);
						Dfs(tmp2,1);
						da[tot]=Data(node[tmp2].t2,tmp5,tmp6);
						da2[tot++]=Data(node[tmp2].d2,tmp5,tmp6);
					}
				}
				if (tmp.n2>0){
			...
			now.t3 = Solve2(tot, da);
			now.d3 = Solve2(tot, da2, now.p1.Score(now.r));
		}	
	}
...
public:
	...
	double Prb_win(){return node[1].t2;}
	
	double Prb_scr(){return node[1].d2;}
	
	int Action(){return node[1].ga;}
};
